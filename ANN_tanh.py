...
#  - Multi-layer 'tanh' ANN models for binary classification problems
...
class ANN_tanh(object):
	...
	def forward(self, X):
		...
		for i in range(0,len(self.M)):
			self.Z.append(np.tanh(self.Z[i].dot(self.W[i]) + self.b[i]))
		self.Z.append(sigmoid(self.Z[len(self.M)].dot(self.W[len(self.M)]) + self.b[len(self.M)]))
		return self.Z[-1]
