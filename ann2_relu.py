...
for i in range(Nsteps):	
	...
	W1 += alpha * Z1.T.dot((Y-PY).dot(W2buf.T) * 0.5*(1+np.sign(Z2)))
	b1 += alpha * ((Y-PY).dot(W2buf.T) * 0.5*(1+np.sign(Z2))).sum(axis=0)
	W0 += alpha * X.T.dot(((Y-PY).dot(W2buf.T) * 0.5*(1+np.sign(Z2))).dot(W1buf.T) * 0.5*(1+np.sign(Z1)))
	b0 += alpha * (((Y-PY).dot(W2buf.T) * 0.5*(1+np.sign(Z2))).dot(W1buf.T) * 0.5*(1+np.sign(Z1))).sum(axis=0)
	if i % 100 == 0:
