...
for i in range(Nsteps):	
	...
	PY = forward_step_sigmoid(Z2, W2, b2)
	J[i] = cross_entropy_bin(Y, PY)
	W1buf = W1
	W2buf = W2
	W2 += alpha * Z2.T.dot(Y-PY)
	b2 += alpha * (Y-PY).sum(axis=0)
	W1 += alpha * Z1.T.dot((Y-PY).dot(W2buf.T) * (Z2 * (1-Z2)))
	b1 += alpha * ((Y-PY).dot(W2buf.T) * (Z2 * (1-Z2))).sum(axis=0)
	W0 += alpha * X.T.dot(( (Y-PY).dot(W2buf.T) * (Z2 * (1-Z2)) ).dot(W1buf.T) * (Z1 * (1-Z1)) )
	b0 += alpha * (( (Y-PY).dot(W2buf.T) * (Z2 * (1-Z2)) ).dot(W1buf.T) * (Z1 * (1-Z1))).sum(axis=0) 
	if i % 100 == 0:
